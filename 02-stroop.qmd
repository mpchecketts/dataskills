...
There is a walkthrough video of this chapter available via [OneDrive](https://gla-my.sharepoint.com/:v:/g/personal/matthew_checketts_glasgow_ac_uk/IQBuiEgQ4bzGQbP-IYT0j4iLAVToEEvutkAWBxz8ydQxaZw?e=8L0kIX). We recommend first trying to work through each section of the book on your own and then watching the video if you get stuck, or if you would like more information. This will feel slower than just starting with the video, but you will learn more in the long-run. Please note that there may have been minor edits to the book since the video was recorded. Where there are differences, the book should always take precedence.
...
`r mcq(c("Color perception", "Language comprehension", answer = "Cognitive interference", "Reading speed"))`
...
`r mcq(c("People are naturally slower at reading than identifying colors", answer = "Reading is an automatic process that can interfere with color identification", "Color identification is an automatic process that can interfere with reading", "None of the above"))`
...
3. In a typical Stroop test, `r fitb("congruent",ignore_case = TRUE)` trials (where the color of the ink matches the word) are completed faster than `r fitb("incongruent", ignore_case = TRUE)` trials (where the color of the ink does not match the word).
...
```{r img-stroop-project, echo=FALSE, fig.cap="Creating a project for the Stroop Effect"}

knitr::include_graphics("images/stroop/new-project.gif")
...
```{r img-unzip, echo=FALSE, fig.cap="Unzipping and moving the files"}

knitr::include_graphics("images/stroop/unzip.gif")
...
* `participant_data.csv` is a data file that contains each participant's anonymous ID, age, and gender. This data is in `r glossary("wide", "wide-form")` which means that all of the observations about one subject are in the same row. There are 270 participants, so there are 270 rows of data.

```{r fig.cap= "Participant info data", echo = FALSE}
head(ppt_info)
```

* `experiment_data.csv` is a data file that contains each participant's anonymous ID, and mean reaction time for all the congruent and incongruent trials they completed. This data is in `r glossary("long", "long-form")` where each observation is on a separate row so for the Stroop experiment, each participant has two rows because there are two observations (one for congruent trials and one for incongruent trials). So there are 270 participants, but 540 rows of data (270 * 2).
...
Add-on packages are not included with base R, but have to be downloaded and installed from an archive, in the same way that you would, for instance, download and install PokemonGo on your smartphone. The main repository where packages reside is called `r glossary("CRAN")`, the Comprehensive R Archive Network.
...
```{r img-install, echo=FALSE, fig.cap="Installing packages"}

knitr::include_graphics("images/stroop/install.gif")
...
```{r library-pckg, filename="Loading a package"}
library(cowsay)
```
...
Now you can use the function `say()`. A `r glossary("function")` is a name that refers to code that performs some sort of action.
...
```{r cowsay-demo, filename="Using a function"}
say()
```

After the function name, there is a pair of parentheses, which contain zero or more `r glossary("argument", "arguments")`. These are options that you can set. If you don't give it any information, it will try and use the default arguments if it has them. `say()` has two main arguments with a `r glossary("default value")`: `what` the text says (default `Hello world`), and the animal the message is said `by` (default is a cat).
...
```{r cowsay-demo-2, eval = FALSE, filename="Adding an argument"}
...
If you want more information about what a function does or how to use it, you can look at the help document. If a `r glossary("function")` is in `r glossary("base R")` or a package you have loaded, you can type `?function_name` in the console to access the help file. At the top of the help it will give you the function and package name. 
...
- `stringr`, for `r glossary("string", "strings")`
- `forcats`, for `r glossary("factor", "factors")`
...
```{r img-tidyverse, echo=FALSE, fig.cap="Tidyverse message when successfully loaded"}

knitr::include_graphics("images/stroop/tidyverse.png")
...
What is the **mean** age pf participants to 1 decimal place? `r fitb(24.4)`

What is the **mean** overall reaction time to 1 decimal place? `r fitb(c(817.7))`
...
```{r fig.cap = "Number of participants by gender"}
...
Are there more men, women, or non-binary participants in the sample? `r mcq(c("More men", answer = "More women", "More non-binary participants"))`

* Copy, paste, and run the below code  in code chunk 7 to create violin-boxplots of reaction times for each condition.

```{r, fig.cap= "Violin-boxplot of reaction times in each condition"}
...
Which condition has the longest median reaction time? `r mcq(c("Congruent", answer = "Incongruent"))`
